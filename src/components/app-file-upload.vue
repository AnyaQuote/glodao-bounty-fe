<template>
  <v-sheet outlined class="d-flex align-center rounded px-5">
    <div v-if="isImageFile" class="mr-5">
      <v-img v-if="source" :src="source" class="rounded" height="50" width="50" contain />
      <v-img v-else :src="require('@/assets/icons/file-logo.svg')" max-height="42" max-width="42" contain />
    </div>
    <v-file-input
      class="py-5"
      show-size
...
export default class AppUploadField extends Vue {
  ...
  config: FileConfig = {
    ...
    hint: this.isImageFile ? 'Allow file types: png, jpg' : 'Allowed file types: csv, txt',
    accept: this.isImageFile ? '.png,.jpg' : '.csv,.txt',
    isEdit: !!this.data,
    error: '',
  }
